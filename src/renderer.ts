...
const tabScroller = document.querySelector('.tab-scroller') as HTMLElement;
const explorerSection = document.querySelector('.explorer-section') as HTMLElement;
...
let openFiles: { path: string; name: string; state?: EditorState }[] = [];
...
// Resizer logic
function setupResizer(resizerId: string, targetId: string, type: 'col' | 'row', isReverse: boolean = false) {
    ...
    resizer.addEventListener('mousedown', (e) => {
        ...
        const onMM = (ev: MouseEvent) => {
            ...
            if (type === 'col') { target.style.width = `${nSize}px`; target.style.flex = 'none'; }
            else { target.style.height = `${nSize}px`; }
        };
...
function addMessage(role: 'user' | 'ai', content?: string): HTMLElement {
    ...
    if (content) bubble.innerText = content;
...
queryInput.addEventListener('input', () => {
    ...
    } else { mentionsDropdown.style.display = 'none'; }
});

function showMentions(q: string) {
...
queryInput.addEventListener('drop', (e) => {
    ...
    if (path) { queryInput.value += ` @${path.split(/[\\/]/).pop()} `; }
});
...
async function initWorkspace() {
    ...
    fileTree.innerHTML = '';
    workspaceFileCache = [];
    if (activeWorkspacePath) { await renderFolder(activeWorkspacePath, fileTree); updateGitStatus(); }
}

async function renderFolder(path: string, parent: HTMLElement, depth: number = 0) {
    // @ts-ignore
    const res = await window.codeactAPI.readDir(path);
    if (!res.success) return;
    for (const item of res.items) {
        if (!item.isDirectory) workspaceFileCache.push({ name: item.name, path: item.path });
        const cont = document.createElement('div'); cont.className = 'item-container';
        const el = document.createElement('div'); el.className = 'file-item'; el.draggable = true;
        ...
        cont.appendChild(el);
        const sub = document.createElement('div'); sub.className = 'sub-folder'; sub.style.display = 'none';
        cont.appendChild(sub);
        el.addEventListener('click', async (e) => {
            e.stopPropagation();
            if (item.isDirectory) { if (sub.innerHTML === '') await renderFolder(item.path, sub, depth + 1); sub.style.display = sub.style.display === 'none' ? 'block' : 'none'; }
            else { addTab(item.path, item.name); }
        });
        el.addEventListener('contextmenu', async (e) => {
            e.preventDefault();
            if (confirm(`Delete ${item.name}?`)) { // @ts-ignore
                const r = await window.codeactAPI.deleteItem(item.path); if (r.success) { closeTab(item.path); initWorkspace(); } }
        });
...
function updateTabUI() {
    ...
    openFiles.forEach(f => {
        const t = document.createElement('div'); t.className = `codeic-tab ${f.path === activeFilePath ? 'active' : ''}`;
        t.id = `tab-${btoa(f.path).replace(/=/g, '')}`;
        t.innerHTML = `<span>${f.name}</span><span class="close-tab">×</span>`;
        t.addEventListener('click', () => { if (activeFilePath !== f.path) openFile(f.path); });
        t.querySelector('.close-tab')?.addEventListener('click', (e) => { e.stopPropagation(); closeTab(f.path); });
        tabScroller.appendChild(t);
    });
}

function addTab(p: string, n: string) { if (!openFiles.find(f => f.path === p)) openFiles.push({ path: p, name: n }); openFile(p); }
function closeTab(p: string) {
    const idx = openFiles.findIndex(f => f.path === p); if (idx === -1) return;
    openFiles.splice(idx, 1);
    if (activeFilePath === p) { if (openFiles.length > 0) openFile(openFiles[Math.max(0, idx - 1)].path); else { activeFilePath = null; if (editor) editor.destroy(); editorContainer.innerHTML = '<div style="text-align:center; padding-top:100px; color:#444;">Select a file to begin</div>'; } }
    updateTabUI();
}

async function openFile(p: string) {
    if (activeFilePath && editor) { const cf = openFiles.find(f => f.path === activeFilePath); if (cf) cf.state = editor.state; }
    activeFilePath = p; updateTabUI();
    setTimeout(() => { const tid = `tab-${btoa(p).replace(/=/g, '')}`; const tel = document.getElementById(tid); if (tel) tel.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' }); }, 50);
    const file = openFiles.find(f => f.path === p); if (!file) return;
    if (editor) editor.destroy(); editorContainer.innerHTML = '';
    if (file.state) { editor = new EditorView({ state: file.state, parent: editorContainer }); return; }
    // @ts-ignore
    const res = await window.codeactAPI.readFile(p); if (!res.success) return;
    const ext = p.split('.').pop(); const lang = ext === 'py' ? python() : javascript();
...
async function saveFile() {
    if (!activeFilePath || !editor) return; const c = editor.state.doc.toString(); statusText.innerText = "Saving...";
    // @ts-ignore
    const r = await window.codeactAPI.writeFile(activeFilePath, c); if (r.success) { statusText.innerText = "File Saved"; setTimeout(() => { statusText.innerText = "Ready"; }, 2000); }
}
window.addEventListener('keydown', (e) => { if (e.ctrlKey && e.key === 's') { e.preventDefault(); saveFile(); } });
...
function handleMenuAction(action: string) {
    const handlers: { [key: string]: () => void } = {
        ...
        'closeAll': () => { openFiles = []; activeFilePath = null; if (editor) editor.destroy(); updateTabUI(); },
        'toggleSidebar': () => { const sidebar = document.querySelector('.codeic-sidebar') as HTMLElement; sidebar.style.display = sidebar.style.display === 'none' ? 'flex' : 'none'; },
    };

    handlers[action]?.();
}

function switchTerminalTab(tab: string) {
    document.querySelectorAll('.terminal-tab-item').forEach(t => t.classList.remove('terminal-tab-active'));
    const activeTab = document.querySelector(`.terminal-tab-item[data-tab="${tab}"]`) as HTMLElement;
    if (activeTab) activeTab.classList.add('terminal-tab-active');

    document.querySelectorAll('.terminal-content').forEach(content => content.classList.add('hidden'));
    const activeContent = document.getElementById(tab + 'Pane') as HTMLElement;
...
const fi = new FitAddon(); trm.loadAddon(fi); trm.open(tc); fi.fit();
// @ts-ignore
window.codeactAPI.spawnTerminal();
trm.onData(d => { // @ts-ignore
...
sT.forEach((tab, index) => {
    tab.addEventListener('click', () => {
        const act = tab.classList.contains('active'); const side = document.querySelector('.codeic-sidebar') as HTMLElement; const res = qs('sidebarResizer');
        if (act) { const hid = side.style.display === 'none'; side.style.display = hid ? 'flex' : 'none'; res.style.display = hid ? 'block' : 'none'; if (hid) tab.classList.add('active'); else tab.classList.remove('active'); }
        else { sT.forEach(t => t.classList.remove('active')); tab.classList.add('active'); side.style.display = 'flex'; res.style.display = 'block'; if (index === 0) { explorerSection.style.display = 'block'; sourceControlSection.style.display = 'none'; } else if (index === 1) { explorerSection.style.display = 'none'; sourceControlSection.style.display = 'block'; updateGitStatus(); } }
    });
});
...
runBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (resp.error) {
            (aiResp.querySelector('.final-answer') as HTMLElement).innerHTML = `<span style="color:#ff5555">Error: ${resp.error}</span>`;
        } else {
            (aiResp.querySelector('.final-answer') as HTMLElement).innerText = resp.finalAnswer || "Analysis complete.";
            statusText.innerText = `Complete ${resp.execution_time_ms}ms`;
        }
    } catch (err: any) {
        (aiResp.querySelector('.final-answer') as HTMLElement).innerHTML = `<span style="color:#ff5555">Host Failure: ${err.message}</span>`;
    } finally {
