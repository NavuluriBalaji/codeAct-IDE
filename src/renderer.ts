    ...
    for (const item of res.items) {
        ...
        el.addEventListener('click', async (e) => {
            ...
            if (item.isDirectory) { 
                ...
                if (sub.innerHTML === '') await renderFolder(item.path, sub, depth + 1); 
                const isHidden = sub.style.display === 'none';
                sub.style.display = isHidden ? 'block' : 'none'; 
                if (caret) {
                    if (isHidden) caret.classList.add('open');
                    else caret.classList.remove('open');
                }
            }
